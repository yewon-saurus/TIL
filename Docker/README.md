...
- [Dockerfile](#dockerfile)
    - [Dockerfile에서 명령어 하나가 실패하더라도, Docker build를 중단하지 않고 다음 명령을 이어서 실행하도록 하려면?](#dockerfile에서-명령어-하나가-실패하더라도-docker-build를-중단하지-않고-다음-명령을-이어서-실행하도록-하려면)
...
## Dockerfile에서 명령어 하나가 실패하더라도, Docker build를 중단하지 않고 다음 명령을 이어서 실행하도록 하려면?
...
- ENTRYPOINT와 CMD는 해당 컨테이너가 수행하게 될 실행 명령을 정의하는 선언문입니다. 즉, 컨테이너가 무슨 일을 하는지 결정하는 최종 단계를 정의하는 명령이라고 생각하면 됩니다. 때문에, 보통 Dockerfile의 가장 마지막 부분 쯤에 ENTRYPOINT 또는 CMD를 선언하게 됩니다.
- 둘의 가장 큰 차이점은 "컨테이너 시작 시 실행 명령에 데한 Default 지정 여부"입니다. 만약 ENTRYPOINT를 사용하여 컨테이너 수행 명령을 정의한 경우, 해당 컨테이너가 실행될 때 **반드시 ENTRYPOINT에서 지정한 명령을 수행**합니다. 하지만, CMD를 사용하면 **컨테이너를 실행할 때 인자값을 주게 되었을 때 Dockerfile에 지정된 CMD 값을 지정된 인자값으로 대신 변경하여 실행**합니다. (CMD는 오버라이딩이 쉽다 => docker run 시 다른 실행 명령어가 인자로 주어지면, CMD에 써 준 내용은 무시된다!!)
- (CMD 설명이 이해하기 어렵네요) 각각의 올바른 사용 예를 들어보며 이해해 보겠습니다. 첫째, 컨테이너가 수행될 때 변경되지 않을 실행 명령은 CMD보다는 ENTRYPOINT로 정의하는 게 좋습니다. 컨테이너를 만들 때 아마 대부분은 해당 컨테이너가 실행될 목적이 분명할 것입니다. 즉, 이미지를 만들 때 nginx / node / mysql 같은 메인 프로세스가 될 명령이 있다면, 이것의 실행 명령은 ENTRYPOINT로 정의하는 게 명확할 것입니다. 둘째, 메인 명령어 실행 시 default option 인자 값은 CMD로 정의해 주는 게 좋습니다. 앞선 설명처럼, CMD는 ENTRYPOINT와 함께 사용 시 **추가 인자 값으로 활용**됩니다. 그러므로, 메인 프로세스에 대한 default 옵션값을 CMD로 정의해주면 좋습니다. 마지막으로, 이는 ENTRYPOINT와 CMD 모두 해당되는 사항입니다. 둘은 리스트 포맷(`["args1", "args2", ...]`)으로 정의해 주는 것이 좋습니다. 일반적인 shell 형태로도 작성이 가능하지만, 작성 방식에 따라 다른 방식으로 프로세스가 구동되게 됩니다. (❓ 아직 명확한 차이는 모르겠다..) Docker 공식 Documentation에서도, List format으로 작성하는 것을 추천하고 있습니다.
