...
- `reduce()` 메서드는 배열의 각 요소에 대해 주어진 reducer 함수를 실행하고, 하나의 결과값 '누적 계산 결과 값'을 반환합니다. 리듀서 함수는 네 개의 인자를 가지는데, 순서대로 '누산기 acc', '현재 값 cur', '현재 인덱스 idx', '원본 배열 src' 입니다. 리듀서 함수의 return 값은 누산기에 할당되고, 누산기는 순화 중 유지되므로 결국 최종 결과는 하나의 값이 됩니다.
- `arr.reduce(callback(accumulator, currentValue, currentIndex, array), initialValue)`
- reduce 함수의 초기값이 지정되지 않으면, reduce 함수는 배열의 첫 번째 값을 처리할 때 accumulator 값으로 첫 번째 값(arr[0])을 세팅합니다. 따라서, 가능하면 초기값을 지정하는 편이 실수를 예방하기에 좋습니다.
- reduce 함수와 기능은 동일하면서 '배열의 오른쪽부터' 연산을 수행하는 `reduceRight()` 함수도 있습니다.